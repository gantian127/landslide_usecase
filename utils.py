...
def cal_subsurface_flow_depth(
    soil_depth, soil_water_layer, layer_threshold=[0, 0.07, 0.28, 1, 2], porosity=0.5
):
...
def cal_safety_factor(
    ...
    # left term
    left_term = (
        (root_cohesion + soil_cohesion)
        / (soil_depth * soil_bulk_density * gravity_acceleration)
        / np.sin(slope_angle)
    )
    ...
    right_term = (
        np.cos(slope_angle)
        * tan_fi
        * (1 - relative_wetness * water_density / soil_bulk_density)
        / np.sin(slope_angle)
    )
